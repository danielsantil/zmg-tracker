...
- **Navbar mobile pattern = hamburger dropdown** (mockups approved). The **language selector is
  deferred to v2.8** with i18n, so v2.6 never ships a dead control.
...
Follow-on work is deferred to its own plans: the Terraform-state backend and cold-start work to
**[build-plan-2.7.md](build-plan-2.7.md)** (M39–M42), and the multilingual EN/ES work to
**`build-plan-2.8.md`**. Blast radius (per CLAUDE.md): **M35** API → full `dotnet test`. **M36** API/DTO/migration →
...
selector** (ships in v2.8 with i18n) — but leave layout room for it.
