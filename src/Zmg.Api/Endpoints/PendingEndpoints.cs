...
public static class PendingEndpoints
{
    public static void MapPendingEndpoints(this IEndpointRouteBuilder app)
    {
        ...
        group.MapGet("", async (IPendingService pending, CancellationToken ct) =>
            Results.Ok(await pending.ListAsync(ct)));

        group.MapGet("/{id:guid}", async (Guid id, IPendingService pending, CancellationToken ct) =>
            Results.Ok(await pending.ListByReleaseIdAsync(id, ct)));
    }
}
